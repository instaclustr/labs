...
This script runs for ~40 seconds and then stops on its own.

Run:  python demo/03_detect_async.py
"""
import asyncio
import random
...
async def main() -> None:
    ...
    print(f"streaming for {RUN_SECONDS}s (detection lags ~5s behind — the MV refresh interval)\n")
    stop = asyncio.Event()
